...
public class AccelerometerSensorProvider implements SensorEventListener {
    ...
    public AccelerometerSensorProvider(SensorProvider activity, Context context) {
    ...
    public class SensorMovementRunnable implements Runnable {
        ...
        public void run() {
            removeHandlerCallback();
            try {
                mSensorManager.registerListener(AccelerometerSensorProvider.this, mAccelerometer, SensorManager.SENSOR_DELAY_NORMAL);
            } catch (Exception e) {
            }
            if(move==null)
            {
